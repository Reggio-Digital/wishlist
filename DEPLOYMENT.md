...
1. **Build the image**
   ```bash
   docker build -t <your-dockerhub-username>/wishlist:latest .
   ```
...
4. **Push to Docker Hub**
   ```bash
   docker push <your-dockerhub-username>/wishlist:latest
   ```

5. **Tag versions (recommended)**
   ```bash
   # Tag with version number
   docker tag <your-dockerhub-username>/wishlist:latest \
              <your-dockerhub-username>/wishlist:v1.0.0

   docker push <your-dockerhub-username>/wishlist:v1.0.0
   ```
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -t <your-dockerhub-username>/wishlist:latest \
  --push .
```
...
4. **Start with PM2**
   ```bash
   npm install -g pm2
   pm2 start npm --name wishlist -- start
   pm2 save
   pm2 startup
   ```
...
```bash
# Backup database
docker cp wishlist:/app/data/wishlist.db ./backup-$(date +%Y%m%d).db

# Restore database
docker cp ./backup.db wishlist:/app/data/wishlist.db
...
docker cp wishlist:/app/data/wishlist.db \
